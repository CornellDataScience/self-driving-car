...
def line_search_reset(binary_img, left_lane, right_line):
    ...
    if len(right_line.prevx) > 10:
        ...
        right_fit_plotx = (
            right_avg_fit[0] * ploty**2 + right_avg_fit[1] * ploty + right_avg_fit[2]
        )
    ...
    left_lane.startx, right_line.startx = (
        left_lane.allx[len(left_lane.allx) - 1],
...
def line_search_tracking(b_img, left_line, right_line):
    ...
    leftx_min = (
        left_line_fit[0] * nonzeroy**2
        + left_line_fit[1] * nonzeroy
        + left_line_fit[2]
        - window_margin
    )
    leftx_max = (
    ...
    rightx_min = (
        right_line_fit[0] * nonzeroy**2
        + right_line_fit[1] * nonzeroy
        + right_line_fit[2]
        - window_margin
    )
    rightx_max = (
    ...
    if (
        len(left_line.prevx) > 10
    ):  # take at least 10 previously detected lane lines for reliable average
...
def illustrate_driving_lane(
    img, left_line, right_line, lane_color=(255, 0, 255), road_color=(0, 255, 0)
):
    """
    #---------------------
...
def illustrate_info_panel(img, left_line, right_line):
    ...
    cv2.putText(
        img, "Measurements ", (75, 30), cv2.FONT_HERSHEY_COMPLEX, 0.8, (80, 80, 80), 2
    )
    ...
    cv2.putText(
        img, lane_info, (10, 63), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (100, 100, 100), 1
    )
    cv2.putText(
        img, lane_curve, (10, 83), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (100, 100, 100), 1
    )
    cv2.putText(
        img, deviate, (10, 103), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (100, 100, 100), 1
    )
...
def illustrate_driving_lane_with_topdownview(image, left_line, right_line):
    ...
    img = cv2.imread("lane/ferrari.png", -1)
        ...
        [
            np.flipud(
                np.transpose(
                    np.vstack(
                        [
                            right_plotx + lane_offset - lane_width + window_margin / 4,
                            ploty,
                        ]
                    )
                )
